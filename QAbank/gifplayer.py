...
class playGif():
    ...
    def decomposePics(self):#分解 gif 文件的每一帧到独立的图片文件，存在临时目录中
        ...
        max_size = (300, 300)
        width_factor = max_size[0] / self.__width
        height_factor = max_size[1] / self.__height
        factor = min(width_factor, height_factor)
        new_size = (int(self.__width*factor), int(self.__height*factor))
        
        os.mkdir(self.strTemporaryFolder) #创建临时目录
        for frame in ImageSequence.Iterator(img): #遍历每帧图片
            frame_resize = frame.resize(new_size)
            frame_resize.save(os.path.join(self.strTemporaryFolder,STR_FRAME_FILENAME.format(i+1))) #保存独立图片
            i += 1
    ...
    def getNextFrameIndex(self,frame=0):#返回下一张的帧数序号
        ...
        if frame==self.__intCount:
            ...
            return frame + 1 #下一张
        #
    def playGif(self,tk,widget,time=80):#开始调用自身实现播放，time 单位为毫秒
        img=self.getPicture()
